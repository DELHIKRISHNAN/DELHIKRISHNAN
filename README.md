...
Mostly Python and JavaScript day to day.<br>
On the security side: `nmap`, `burp suite`, `metasploit`, `wireshark`, `nuclei`, `splunk`. <br>
For building things: `django` / `react` / `node` + `express`, backed by `firebase` or `postgres`. <br>
The AI/CV side runs on `opencv`, basic `CNN` / `SVM` models, and sentence-BERT for NLP work. <br>
Daily driver is Kali on Linux, Ubuntu on the Pi boards.<br>
